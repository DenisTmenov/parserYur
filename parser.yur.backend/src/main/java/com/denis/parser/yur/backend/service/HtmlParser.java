...
public class HtmlParser {
			...
			if (!selectA.isEmpty()) {
				...
				for (Element element : selectA) {
					...
					if (href.contains(".")) {
						...
						doorImage.setTipe(split[split.length - 1]);
					}
	...
	private void saveDoorImageToDisk(Door door) {
		byte[] bs = door.getDoorImages().get(0).getImage();

		try {
			Files.write(Paths
					.get("D://" + door.getDoorImages().get(0).getName() + "." + door.getDoorImages().get(0).getTipe()),
					bs, StandardOpenOption.CREATE);
